...
class DriverInstaller:
    ...
    def _build_and_install_driver(self, driver_dir: Path):
        ...
        for kernel_build_dir in kernel_dirs:
            ...
            # Build
            self._run_command(["make", f"KERNELRELEASE={kernel}"], cwd=driver_dir)

            # Install
            self._run_command(
                ["sudo", "make", "install", f"KERNELRELEASE={kernel}"], cwd=driver_dir
            )

            # Clean
            self._run_command(
                ["make", "clean", f"KERNELRELEASE={kernel}"], cwd=driver_dir
            )

    def _git_clone_or_update(
        self, repo_url: str, target_dir: Path, branch: str = "master"
    ) -> bool:
        ...
        if target_dir.exists():
            ...
            result = self._run_command(
                ["git", "pull"], cwd=target_dir, capture_output=True, text=True
            )
    ...
    def install_sound_driver(self, sound_card: Optional[str] = None) -> bool:
        ...
        if sound_card == "hifiberry":
            logger.info(
                "HiFiBerry detected (Maker Faire 2018), no WM8960 installation needed"
            )
            return False

        driver_dir = self.inst_dir / "wm8960"
        changed = self._git_clone_or_update(
            "https://github.com/pguyot/wm8960", driver_dir, branch="tagtagtag-sound"
        )
    ...
    def install_ears_driver(self) -> bool:
        ...
        changed = self._git_clone_or_update(
            "https://github.com/pguyot/tagtagtag-ears", driver_dir, branch="master"
        )
    ...
    def install_rfid_drivers(self) -> bool:
        ...
        changed = self._git_clone_or_update(
            "https://github.com/pguyot/cr14", cr14_dir, branch="master"
        )
        ...
        changed = self._git_clone_or_update(
            "https://github.com/pguyot/st25r391x", st25_dir, branch="main"
        )
        ...
        if changed or not st25_dir.exists():
            ...
            if config_path.exists():
                ...
                if "dtoverlay=st25r391x" in config and not config.count(
                    "#dtoverlay=st25r391x"
                ):
...
Examples:
  ...
  sudo python -m scripts.setup_drivers --dry-run
    ...
    parser.add_argument(
        "--upgrade", action="store_true", help="Upgrade existing drivers"
    )
    ...
    parser.add_argument(
        "--sound", action="store_true", help="Install only sound driver"
    )

    parser.add_argument("--ears", action="store_true", help="Install only ears driver")

    parser.add_argument(
        "--rfid", action="store_true", help="Install only RFID/NFC drivers"
    )
    ...
    if detector.is_pi_zero():
        ...
        logger.warning(
            "Not a Pi Zero - installation may work but is not officially supported"
        )
