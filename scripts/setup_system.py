...
class SystemSetup:
    ...
    def setup_postgresql(self):
        ...
        if result.returncode != 0:
            ...
            for pg_hba in pg_hba_files:
                ...
                self._run_command([
                    'sudo', 'sed', '-i',
                    '-E', r's/^(local +all +all +)peer$/\1trust/',
                    str(pg_hba)
                ])
    ...
    def compile_localization(self):
        ...
        locales = [
            '-l', 'fr_FR', '-l', 'de_DE', '-l', 'en_US', '-l', 'en_GB',
            '-l', 'it_IT', '-l', 'es_ES', '-l', 'ja_jp', '-l', 'pt_BR',
            '-l', 'de', '-l', 'en', '-l', 'es', '-l', 'fr',
            '-l', 'it', '-l', 'ja', '-l', 'pt'
        ]
        ...
        for module_dir in self.root_dir.glob('nab*/'):
            ...
            if locale_dir.exists():
                ...
                self._run_command(
                    [str(django_admin), 'compilemessages'] + locales,
                    cwd=module_dir
                )
...
"""
    ...
    def setup_avahi(self):
        ...
        pynab_service = Path('/etc/avahi/services/pynab.service')
...
"""
            with open('/tmp/pynab.service', 'w') as f:
                f.write(service_xml)
        ...
        # NabBlockly service
        nabblockly_service = Path('/etc/avahi/services/nabblocky.service')
...
"""
            with open('/tmp/nabblocky.service', 'w') as f:
                f.write(service_xml)
            
            self._run_command(['sudo', 'mv', '/tmp/nabblocky.service', str(nabblockly_service)])
...
Examples:
  ...
  sudo python -m scripts.setup_system --dry-run
    ...
    parser.add_argument(
        '--database',
        action='store_true',
        help='Setup PostgreSQL database only'
    )
    
    parser.add_argument(
        '--nginx',
        action='store_true',
        help='Setup Nginx only'
    )
    ...
    parser.add_argument(
        '--services',
        action='store_true',
        help='Setup systemd services only'
    )
    ...
    try:
        ...
        setup_all = not (args.database or args.nginx or args.migrations or args.services)
